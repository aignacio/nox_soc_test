// UART smoke test for NoX SoC (RV32I)
// - Initializes SP/GP, minimal C runtime
// - Enables UART and transmits a short string
// - Reports pass/fail via UVM handshake addresses

#include <stdint.h>
#include "../include/soc_mmap.h"
#include "../include/uvm_test.h"

extern unsigned long _stack_top;
void main(void);

__attribute__((naked, section(".text.entry"))) void _start(void) {
    __asm__ volatile (
        "la sp, _stack_top\n\t"
        "la gp, __global_pointer$\n\t"
        "jal ra, main\n\t"
        "1: wfi; j 1b\n\t"
    );
}

static inline void uart_enable(void) {
    REG32(UART_CTRL) = UART_CTRL_EN;
}

static inline int uart_tx_ready(void) {
    return (REG32(UART_STATUS) & UART_STATUS_TX_READY) != 0;
}

static void uart_putc(char c) {
    // Wait for TX ready with a simple timeout to avoid hang
    for (uint32_t t = 0; t < 100000u; ++t) {
        if (uart_tx_ready()) {
            REG32(UART_TXDATA) = (uint32_t)(uint8_t)c;
            return;
        }
    }
    REPORT_FAIL(0x01); // TX not ready
}

static void uart_puts(const char *s) {
    while (*s) {
        uart_putc(*s++);
    }
}

void main(void) {
    uart_enable();
    busy_delay(1000);
    uart_puts("Hello, NoX!\n");
    REPORT_PASS();
}

